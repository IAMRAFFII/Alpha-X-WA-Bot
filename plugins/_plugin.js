...
var LANG = {
            unaffinfo: Config.LANG == 'SI' || Config.LANG == 'EN' ? '*ස්ථාපිත ප්ලගිනයේ අනතුරුදායක මට්ටම:* _%' : '*Danger Level of Installed Plugin:* _%',
            ...
            limit: Config.LANG == 'SI' || Config.LANG == 'EN' ? '*මෙම ප්ලගිනය ආරක්‍ෂක සීමාව ඉක්මවයි!*\n*Zararlılık Yüzdesi:* _%' : '*This Plugin Exceeds Security Limit!*\n*Percentage of Harm:* _%',
            imside: Config.LANG == 'SI' || Config.LANG == 'EN' ? '*ඔබට දැනට පවතින ප්ලගීන නැවත පූරණය කළ නොහැක!*' : '*You Cant Reinstall Existing Plugins!*'
};
Alpha.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install https://gist.github.com/SL-Alpha-X-DEV/4232b1c8c4734e1f06c3d991149c6fbd', MessageType.text)
    try {
...
Alpha.addCommand({pattern: 'plugin$', fromMe: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
...
Alpha.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {

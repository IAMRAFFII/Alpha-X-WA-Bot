...
Asena.addCommand({pattern: 'varset ?(.*)', fromMe: true, desc: Lang.SET_DESC}, (async (message, match) => {

    if (match[1] == '') {
        return await message.client.sendMessage(message.jid, Lang.SET_NONE, MessageType.text); 
    }
    ...
    else if (match[1] == 'password' && message.reply_message) {
        await message.client.sendMessage(message.jid, Lang.SUCC, MessageType.text);
        await new Promise(r => setTimeout(r, 1200));
        await message.client.sendMessage(message.jid, Lang.SUCC_AF, MessageType.text);
        await heroku.patch(baseURI + '/config-vars', { 
            body: { 
                ['BOT_PASSWORD']: message.reply_message.text
            } 
        });
    }
...
Asena.addCommand({pattern: 'poweroff$', fromMe: true, desc: Lang.SHUTDOWN_DESC}, (async(message, match) => {
...
Asena.addCommand({pattern: 'setvar ?(.*)', fromMe: true, desc: Lang.SETVAR_DESC}, (async(message, match) => {
    ...
    if (match[1].match(/94772978164/i)) {

...
var USA_USER = ''
var OTHER = ''
if (CON.LANG == 'SI') ADMIN_USER = '*පරිපාලක ගණන:*', USER_USER = '*සාමාජිකයින් ගණන:*', SRI_USER ='*ශ්‍රී ලංකා සාමාජික සංඛ්‍යාව*', HI_USER = '*ඉන්දියානු සාමාජිකයින් ගණනd *', AZ_USER ='*අසර්බෙයිජාන් සාමාජික සංඛ්‍යාව*', TR_USER = '*තුර්කි සාමාජික සංඛ්‍යාව*', RU_USER ='*රුසියානු සාමාජික සංඛ්‍යාව*', USA_USER ='*USA සාමාජික සංඛ්‍යාව*', OTHER = '*අනෙකුත් සාමාජිකයින්*'
...
if (CON.WORKTYPE == 'private') {
    Asena.addCommand({ pattern: 'details$', fromMe: true, desc: Lang.PL_DESC }, async (message, match) => { 
    ...
    Asena.addCommand({ pattern: 'details$', fromMe: false, desc: Lang.PL_DESC }, async (message, match) => { 
        if (message.jid.includes('-')) {
            var json = await message.client.groupMetadataMinimal(message.jid) 
            var code = await message.client.groupInviteCode(message.jid)
            var nwjson = await message.client.groupMetadata(message.jid) 
